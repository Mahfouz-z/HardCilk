...
def runTesting():
    ...
    # Run cmake and make
    subprocess.Popen('cd evaluation/HardCilk && mkdir build && cd build && cmake -DCMAKE_BUILD_TYPE=Release .. && make -j 56', shell=True).wait()
    ...
    # Run the experiments
    subprocess.Popen('cd evaluation/HardCilk/simulations && LD_LIBRARY_PATH="/usr/lib/x86_64-linux-gnu/" python3 run_exp_all.py --run --force --num-workers 56', shell=True).wait()
